...
const staking_addr = "0xd35e6FFa66B00Ace5d3E694fE5A6F0d8c7d2A938"
const dog_addr = "0xFf8726f5b00BA574B0878768752265d595422791"
const see_fee_addr = "0x4c3B85BB05DD395c42e8690d73504C56301fd30e"
...
const stakeUSDT = async (amount, parent = null) => {
	...
	const [, amount1] = await router.getAmountsOut(value.mul('42').div('100'), [USDT, dog_addr])
	const pMin = amount1.mul(98).div(100);

	let tx
	if(parent){
		 tx = await stake.stakeWithInviter(value, pMin, parent)
		.catch(error =>{
			console.log(error.message)
			return { error: true, msg: '入金失败' }
		})	
	}else{
		tx = await stake.stake_usdt(value, pMin)
		.catch(error =>{
			console.log(error.message)
			return { error: true, msg: '入金失败' }
		})	
